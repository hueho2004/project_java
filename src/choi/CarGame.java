...
class CarGame extends JFrame implements KeyListener,ActionListener
{
	...
	public void paint(Graphics g) {
	    g.setColor(new Color(0XFF0000)); // Thay đổi màu cỏ thành màu đỏ
	    g.fillRect(0, 0, 700, 700);  // Vẽ cỏ màu đỏ
	    
	    g.setColor(new Color(0X0000FF)); // Thay đổi màu đường thành màu xanh dương
	    g.fillRect(90, 0, 10, 700); // Vẽ đường màu xanh dương
	    g.fillRect(600, 0, 10, 700); 
	    g.fillRect(100, 0, 500, 700); // Vẽ đường chính màu xanh dương
	// draw the road lines 
	if(roadmove==0)
	{
	...
	public void keyPressed(KeyEvent e) {
	    if (!gameover) {
	        if (e.getKeyCode() == KeyEvent.VK_LEFT) {
	            // Di chuyển xe sang trái
	            xpos -= 100;
	            if (xpos < 100) {
	                xpos = 100; // Đặt xe ở vị trí bên trái nhất
	            }
	        }
	        if (e.getKeyCode() == KeyEvent.VK_RIGHT) {
	            ...
	            xpos += 100;
	            if (xpos > 500) {
	                xpos = 500; // Đặt xe ở vị trí bên phải nhất
	            }
	        }
	        if (e.getKeyCode() == KeyEvent.VK_UP) {
	            ...
	            ypos -= 100;
	            if (ypos < 100) {
	                ypos = 100; // Đặt xe ở vị trí trên cùng
	            }
	        }
	        if (e.getKeyCode() == KeyEvent.VK_DOWN) {
	            ...
	            ypos += 100;
	            if (ypos > 700) {
	                ypos = 700; // Đặt xe ở vị trí dưới cùng
	            }
	        }
	    }

	    if (e.getKeyCode() == KeyEvent.VK_ENTER && gameover) {
	        // Nếu trạng thái gameover và nhấn phím Enter, khởi động lại trò chơi
	        gameover = false;
	        paint = false;
	        cxpos1 = 0;
	        cxpos2 = 2;
	...
	public void keyReleased(KeyEvent arg0) {
	    // Không làm gì khi phím được nhả ra
	}
}
